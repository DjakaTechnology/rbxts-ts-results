...
interface BaseResult<T, E> {
    ...
    /**
     * Converts from `Result<T, E>` to `T | undefined`, discarding the error if any
     */
...
export class ErrImpl<E> implements BaseResult<never, E> {
	...
	expect(msg: string): never {
		throw `${msg} - Error: ${toString(this.val)}\n${this._stack}`;
	}
	...
	unwrap(): never {
		throw `Tried to unwrap Error: ${toString(this.val)}\n${this._stack}`;
	}
...
export namespace Result {
	 ...
	 */
	export function all<T extends Result<any, any>[]>(...results: T): Result<ResultOkTypes<T>, ResultErrTypes<T>[number]> {
		const okResult = [];
	 ...
	 */
	export function any<T extends Result<any, any>[]>(...results: T): Result<ResultOkTypes<T>[number], ResultErrTypes<T>> {
		const errResult = [];
